...
// Export des potentiels à partir du fichier potentiel_lj.csv
...
//  Écriture d'un fichier VTK (.vtp) pour chaque frame
//  Format VTK XML PolyData lisible par ParaView .
//  Chaque fichier contient :
//    - les positions 3D des particules  
//    - la norme de la vitesse
//    - le vecteur vitesse
//    - l'identifiant

void ecrire_vtk(const Univers& univers, int frame_index,double t, const std::string& dir) {
...
//  Écriture du fichier PVD , indice temporel pour ParaView, il permet de charger 
// toute la simulation d'un coup et d'avoir l' axe du temps .
void ecrire_pvd(const std::vector<double>& temps_frames,const std::string& dir) {
...
int main() {
    ...
    // Univers 2D avec grille de cellules
    Univers U(2, EPSILON, SIGMA, RCUT, {250.0, 200.0, 1.0});

    // Les deux blocs de particules
    int id = 0;
    ...
    // Bloc bleu : 160x40 particules sans vitesse
    ...
    while (t < T_END) {
        // forces Lj mise à jour cellules
    ...
    // Indice temporele dans le fichier pvd
