...
void creer_rectangle(Univers& U,
    ...
    for (int i = 0; i < nx; ++i) {
        for (int j = 0; j < ny; ++j) {
            Particule p;
            p.position = Vecteur(x_min + i * EQ_DIST,
                                 y_min + j * EQ_DIST,
                                 0.0);
            p.vitesse  = v0;
...
int main() {
    ...
    creer_rectangle(U, x_rouge, y_rouge,
                    nx_r, ny_r,
                    Vecteur(0, -10, 0), id);

    // bloc bleu : 160x40 particules, immobile
    creer_rectangle(U, 0.0, 0.0,
                    160, 40,
                    Vecteur(0, 0, 0), id);
    ...
    // En-tete pour que le script Python sache combien de particules
    f_frames << "# N=" << id
             << " dt=" << DT
             << " rcut=" << RCUT << "\n";
    ...
    while (t < T_END) {
        ...
        // Q5- mettre_a_jour_cellules() est appelé dans avancer()
